...
function showSignature() {
  ...
  signature.textContent = firma ? decodeURIComponent(firma) : "Con amor, Marcelo";
  signature.classList.add('visible');

  // --- NUEVO: Iniciar carrusel después de la firma ---
  setTimeout(startPhotoCarousel, 3000); 
}

function startPhotoCarousel() {
  const carousel = document.getElementById('photo-carousel');
  const tree = document.getElementById('tree-container');
  const text = document.getElementById('dedication-text');
  
  // Ocultamos el árbol y el texto para que se vean las fotos
  ...
  setTimeout(() => {
    ...
    function nextImage() {
      ...
      // Cambia de foto cada x segundos (ajustable)
...
function startFloatingObjects () {
  ...
  function spawn () {
    ...
    // Generar más objetos
    if (count++ < 100) setTimeout(spawn, 350 + Math.random() * 500)
...
function showCountdown () {
  ...
  let startDate = startParam
    ...
    : new Date('2025-01-18T00:00:00')
  let eventDate = eventParam
    ? new Date(eventParam + 'T00:00:00')
    : new Date('2026-01-18T00:00:00')
